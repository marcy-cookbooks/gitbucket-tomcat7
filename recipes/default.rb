...
execute "copy-to-webapps" do
  command <<-EOH
  cp -p /usr/local/src/gitbucket/gitbucket.#{node['gitbucket-tomcat7']['version']}.war /usr/share/tomcat7/webapps/gitbucket.war
  rm -rf /usr/share/tomcat7/webapps/gitbucket
  EOH
  action :nothing
